...
alter table public.symbol_indicator_snapshot
    add column if not exists avg_volume_20 numeric,
    add column if not exists is_new_high_50 boolean not null default false;

create index if not exists idx_symbol_metadata_return_on_equity
    on public.symbol_metadata (return_on_equity);

create index if not exists idx_symbol_metadata_debt_to_equity
    on public.symbol_metadata (debt_to_equity);

create index if not exists idx_snapshot_avg_volume_20
    on public.symbol_indicator_snapshot (timeframe, avg_volume_20);

create index if not exists idx_snapshot_is_new_high_50
    on public.symbol_indicator_snapshot (timeframe, is_new_high_50);

create or replace function public.screener_metadata_rule_matches(
    rule jsonb,
    md public.symbol_metadata
) returns boolean
language sql
immutable
as $$
select case
    when md is null then false
    when coalesce(rule->>'field', '') = 'return_on_equity' and coalesce(rule->>'operator', '') = 'gt'
        then md.return_on_equity is not null and md.return_on_equity > (rule->>'value')::numeric
    when coalesce(rule->>'field', '') = 'debt_to_equity' and coalesce(rule->>'operator', '') = 'lt'
        then md.debt_to_equity is not null and md.debt_to_equity < (rule->>'value')::numeric
    else false
end;
$$;
...
select case
    ...
    when coalesce(rule->>'field', '') = 'is_new_high_50' then coalesce(snap.is_new_high_50, false)
    ...
    when coalesce(rule->>'field', '') = 'rsi_14' and coalesce(rule->>'operator', '') = 'gte'
        then snap.rsi_14 is not null and snap.rsi_14 >= (rule->>'value')::numeric
    when coalesce(rule->>'field', '') = 'avg_volume_20' and coalesce(rule->>'operator', '') = 'gt'
        then snap.avg_volume_20 is not null and snap.avg_volume_20 > (rule->>'value')::numeric
    when coalesce(rule->>'field', '') = 'avg_volume_20' and coalesce(rule->>'operator', '') = 'lt'
        then snap.avg_volume_20 is not null and snap.avg_volume_20 < (rule->>'value')::numeric
