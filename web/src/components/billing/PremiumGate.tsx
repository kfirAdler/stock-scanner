...
export function PremiumGate({ kind }: { kind: GateKind }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-border-strong bg-surface-raised shadow-premium">
      <div
        className="pointer-events-none absolute inset-0 opacity-[0.35] dark:opacity-25"
        style={{
          background:
            "radial-gradient(900px 400px at 10% -20%, var(--color-premium-glow), transparent 55%), radial-gradient(600px 300px at 100% 0%, var(--color-primary-soft), transparent 50%)",
        }}
      />
      <div className="relative px-6 py-10 sm:px-10 sm:py-12 text-center space-y-5 max-w-lg mx-auto">
        <div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl border border-border-strong bg-surface-alt text-premium shadow-inner-premium">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2">
          {kind === "login" ? (
            <Link href="/auth/login">
              <Button variant="lux" size="lg" className="w-full sm:w-auto min-w-[200px]">
                {t("signIn")}
              </Button>
            </Link>
          ) : (
