...
import { patternFilterMatches, type PatternFilter, type PatternPayload } from '@/lib/patterns/types';

export function ScanSummary({ data, filter, requested }: {data:PatternPayload;filter?:PatternFilter;requested:boolean}) {
  const t=useTranslations('patterns');
  const locale=useLocale();
  const strict=data.rows.filter(r=>r.matches.some(m=>patternFilterMatches(m,filter))).length;
  const developing=data.rows.filter(r=>r.developing?.some(d=>patternFilterMatches(d.match,filter))).length;
  const rejected=Object.entries(data.diagnostics?.rejected??{}).sort((a,b)=>b[1]-a[1]);
  return <section className="page-card !p-5 sm:!p-6">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-widest text-success">{t(requested?'scanComplete':'snapshotOverview')}</p><h2 className="mt-2 text-xl font-extrabold">{filter?t(filter)+' · ':''}{t('scanHeadline',{scanned:data.coverage.scanned,matches:strict})}</h2></div><div className="text-xs text-text-muted">{data.updatedAt && <p>{t('completedAt')}: {new Date(data.updatedAt).toLocaleString(locale)}</p>}{data.durationSeconds!==undefined && <p className="mt-1">{t('scanDuration',{seconds:data.durationSeconds})}</p>}</div></div>
