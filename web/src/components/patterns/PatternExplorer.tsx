...
export function PatternExplorer() {
  ...
  const [requestPattern,setRequestPattern] = useState<PatternKind>('ascending_triangle');
  ...
  const strictCount = data?.rows.filter(r=>r.matches.length).length ?? 0;
  ...
  const emptySharedChannel = !requestedJob && kind==='channel' && !search.trim() && !!data?.coverage.scanned;
  const filtered = !!search.trim() || (kind!=='all' && kind!==requestedJob?.pattern);
  function reload() { setRequestedJob(null); setView('strict'); setStatus('loading'); setExpanded(null); setRetry(v=>v+1); }
  ...
  return <div className="page-shell page-stack">
    <section className="flex flex-col gap-5 rounded-3xl border border-border bg-gradient-to-br from-primary-soft via-surface-raised to-surface p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8">
      ...
      <div className="inline-flex shrink-0 self-start gap-1 rounded-xl border border-border bg-surface-raised p-1 sm:self-center">{(['US','TA'] as const).map(m=><button key={m} aria-pressed={market===m} onClick={()=>{if(m!==market){setMarket(m);setRequestedJob(null);setData(null);setStatus('loading');setExpanded(null);setView('strict');}}} className={'rounded-lg px-5 py-2 text-sm font-bold '+(market===m?'bg-primary text-on-primary':'text-text-secondary')}>{t(m)}</button>)}</div>
    </section>
    <PatternRequestPanel market={market} pattern={requestPattern} onPatternChange={setRequestPattern} onViewResults={job=>{setRequestedJob(job);setMarket(job.market);setKind(job.pattern);setSearch('');setView('strict');setExpanded(null);}} />
    {requestedJob && <div className="flex flex-wrap items-center justify-between gap-3 text-sm"><p className="text-text-muted">{t('requestResults')}: <strong className="text-text">{t(requestedJob.pattern)} · {t(requestedJob.market)}</strong></p><button className="font-bold text-primary" onClick={()=>{setRequestedJob(null);setKind('all');setView('strict');setExpanded(null);}}>{t('requestBack')}</button></div>}
    ...
    {displayStatus==='ready' && data && <>
      {data.coverage.total>0 && <ScanSummary data={data} pattern={kind==='all'?undefined:kind} requested={!!requestedJob} />}
      {requestedJob?.pattern==='channel' && <div className="space-y-3"><div className="flex gap-2 border-b border-border" aria-label={t('resultViews')}>
        <button aria-pressed={view==='strict'} onClick={()=>changeView('strict')} className={'border-b-2 px-4 py-3 text-sm font-bold '+(view==='strict'?'border-primary text-primary':'border-transparent text-text-muted')}>{t('strictMatches')} <span className="ms-2 rounded-full bg-surface-alt px-2 py-0.5 tabular-nums">{strictCount}</span></button>
        <button aria-pressed={view==='developing'} disabled={!data.diagnostics} onClick={()=>changeView('developing')} className={'border-b-2 px-4 py-3 text-sm font-bold disabled:cursor-not-allowed '+(view==='developing'?'border-warning text-warning':'border-transparent text-text-muted')}>{t('developingSetups')} <span className="ms-2 rounded-full bg-surface-alt px-2 py-0.5 tabular-nums">{data.diagnostics?developingCount:'—'}</span></button>
      </div>{view==='developing' && <p className="rounded-xl border border-warning/20 bg-warning-soft px-4 py-3 text-sm text-warning">{t('developingExplanation')}</p>}</div>}
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex flex-wrap gap-2">{(['all',...kinds] as const).map(k=><button key={k} onClick={()=>{setKind(k);setView('strict');setExpanded(null);if(requestedJob && k!==requestedJob.pattern){setRequestedJob(null);}if(k!=='all')setRequestPattern(k);}} aria-pressed={kind===k} className={'rounded-full border px-4 py-2 text-sm font-semibold '+(kind===k?'border-primary bg-primary-soft text-primary':'border-border bg-surface-raised text-text-secondary')}>{t(k)}</button>)}</div><input aria-label={t('search')} placeholder={t('search')} value={search} onChange={e=>setSearch(e.target.value)} className="ui-control rounded-xl px-4 py-3 text-sm lg:w-64" /></div>
      {!cards.length ? <div className="page-empty-state py-12 text-center"><h2 className="text-xl font-bold">{t(!data.coverage.total?'waiting':emptySharedChannel?'noChannels':filtered?'empty':view==='developing'?'noDeveloping':'noStrictMatches')}</h2><p className="mx-auto mt-2 max-w-xl text-sm text-text-muted">{t(!data.coverage.total?'waitingBody':emptySharedChannel?'noChannelsBody':filtered?'emptyBody':view==='developing'?'noDevelopingBody':'strictEmptyBody')}</p>{emptySharedChannel && <button className="mt-5 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-on-primary" onClick={()=>{setRequestPattern('channel');document.getElementById('pattern-request-selection')?.focus();document.getElementById('pattern-request-title')?.scrollIntoView({behavior:'smooth',block:'center'});}}>{t('prepareChannelSearch')}</button>}{filtered && !emptySharedChannel && <button className="mt-4 text-primary" onClick={()=>{setKind(requestedJob?.pattern??'all');setSearch('');}}>{t('reset')}</button>}{!filtered && view==='strict' && developingCount>0 && <button className="mt-5 rounded-xl bg-warning-soft px-5 py-3 text-sm font-bold text-warning" onClick={()=>changeView('developing')}>{t('viewDeveloping',{count:developingCount})}</button>}</div> : <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{cards.map(({row,match,developing})=>{const id=row.ticker+':'+match.pattern+':'+view;const open=expanded===id;return <article key={id} className={'page-card !p-5 '+(open?'md:col-span-2 xl:col-span-3':'ui-elevated-hover')}>
        ...
        {open && <div className="mt-5"><SetupChart key={(requestedJob?.id??'snapshot')+id} snapshot={row} pattern={match.pattern} jobId={requestedJob?.id} view={view} /></div>}
      </article>;})}</div>}
