...
export function PatternExplorer() {
  ...
  return <div className="page-shell page-stack">
    ...
    {displayStatus==='ready' && data && <>
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex flex-wrap gap-2">{filters.map(item=><button key={item} onClick={()=>{setFilter(item);setExpanded(null);}} aria-pressed={filter===item} className={'rounded-full border px-4 py-2 text-sm font-semibold '+(filter===item?'border-primary bg-primary-soft text-primary':'border-border bg-surface-raised text-text-secondary')}>{t(item)}</button>)}</div><div className="flex flex-wrap items-center gap-2"><div role="group" aria-label={t('resultViews')} className="inline-flex rounded-xl border border-border bg-surface-raised p-1"><button onClick={()=>changeView('strict')} aria-pressed={view==='strict'} className={'rounded-lg px-3 py-2 text-xs font-bold '+(view==='strict'?'bg-primary text-on-primary':'text-text-secondary')}>{t('strictMatches')}</button><button onClick={()=>changeView('developing')} aria-pressed={view==='developing'} className={'rounded-lg px-3 py-2 text-xs font-bold '+(view==='developing'?'bg-warning text-white':'text-text-secondary')}>{t('developingSetups')} ({developingCount})</button></div><input aria-label={t('search')} placeholder={t('search')} value={search} onChange={e=>setSearch(e.target.value)} className="ui-control rounded-xl px-4 py-3 text-sm lg:w-64" /></div></div>
      {!cards.length ? <div className="page-empty-state py-12 text-center"><h2 className="text-xl font-bold">{t(!data.coverage.total?'waiting':emptySharedChannel?'noChannels':filtered?'empty':view==='developing'?'noDeveloping':'noStrictMatches')}</h2><p className="mx-auto mt-2 max-w-xl text-sm text-text-muted">{t(!data.coverage.total?'waitingBody':emptySharedChannel?'noChannelsBody':filtered?'emptyBody':view==='developing'?'noDevelopingBody':'strictEmptyBody')}</p>{filtered && !emptySharedChannel && <button className="mt-4 text-primary" onClick={()=>{setFilter('all');setSearch('');}}>{t('reset')}</button>}{!filtered && view==='strict' && developingCount>0 && <button className="mt-5 rounded-xl bg-warning-soft px-5 py-3 text-sm font-bold text-warning" onClick={()=>changeView('developing')}>{t('viewDeveloping',{count:developingCount})}</button>}</div> : <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{cards.map(({row,match,developing})=>{const id=row.ticker+':'+match.pattern+':'+view;const open=expanded===id;const badgeKey=match.pattern==='channel'?`channel_${channelDirectionOf(match)}`:match.pattern;return <article key={id} className={'page-card !p-5 '+(open?'md:col-span-2 xl:col-span-3':'ui-elevated-hover')}>
        <div className="flex items-start justify-between gap-3"><div><Link href={'/ticker/'+encodeURIComponent(row.ticker)} className="link-hover text-xl font-extrabold" dir="ltr">{row.ticker}</Link><p className="mt-1 text-xs text-text-muted">{row.company_name??t(row.market)}</p></div><div className="flex flex-wrap justify-end gap-1">{match.change && match.change!=='stable' && <span className="rounded-full bg-success-soft px-3 py-1 text-xs font-bold text-success">{t(('change_'+match.change) as 'change_new')}</span>}<span className={'rounded-full px-3 py-1 text-xs font-bold '+(developing?'bg-warning-soft text-warning':'bg-primary-soft text-primary')}>{t(developing?'developingBadge':badgeKey)}</span></div></div>
        {!open && <PatternPreview preview={row.preview} match={match} breachIndices={developing?.breachIndices} />}
        {developing?.breachCount!==undefined && developing.maxBreachPercent!==undefined && <div className="my-4 rounded-xl border border-warning/20 bg-warning-soft p-3 text-xs text-warning"><p className="font-bold">{t('missedBoundaryRule')}</p><p className="mt-1">{t('breachDetail',{count:developing.breachCount,percent:number(developing.maxBreachPercent)})}</p></div>}
